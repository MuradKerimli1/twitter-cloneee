...
const UpdatePackage = ({ close, data }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await Axios({
        ...summaryApi.setUpdatePackage(data.id),
        data: packageData,
      });

      if (res.data.success) {
        const updatedPackagesData = packages.map((pkg) =>
          pkg.id === data.id ? res.data.package : pkg
        );
  ...
  return (
    <section className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 overflow-y-auto">
      <div className="relative w-full max-w-2xl bg-[#15202B] p-6 rounded-xl border border-[#2F3336] max-h-[90vh] overflow-y-auto">
        ...
        {/* Header */}
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-white">Update Package</h1>
        </div>
